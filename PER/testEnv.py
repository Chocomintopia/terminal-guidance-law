...
class simulitiveEnv(object):
    ...
    def _step(self, action):
        ...
        done = self._is_end_state()  # 判断是否是中止状态 获得一个bool值,如果导弹与目标距离<3m，代表打中，done是true
        ...
        训练时在这里修改目标的机动方式
        '''
        self.Value_direction_maneuver.append(np.random.randint(1, 4)) # 随机机动
        # self.Value_direction_maneuver.append(1) # 向上机动
        # self.Value_direction_maneuver.append(2) # 向下机动
        # self.Value_direction_maneuver.append(3) # 正弦机动
        ...
        # # 非线性奖励 R_t = -(r_t/r_0)^0.3
        # # r_flag = self.r0
        # r_flag = self.r[-1]
        # if not done:
        #     delta = -np.power(self.zem_relative[-1]/r_flag, 0.3)
        #     # delta = -np.power(self.r[-1]/r_flag, 0.3)
        #     self.interrupt_flag = False
        #     # delta = -np.power(self.r[-1]/r_flag, 0.2)
        #     # delta = -(self.r[-1]/r_flag)
        # else:
        #     delta = 10
        #     self.done_flag = 0
        ...
        return state, delta, done, self.interrupt_flag
