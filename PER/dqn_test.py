...
from dqn_train import Normalizer, DQN
import torch
import random
...
def test(maneuver_mode, test_num):
    ...
    Value_target_acceleration = 5 # 原来是5. 训练时候 目标加速度是2，等会可以更改试试
    ...
    for i in range(test_num):
        ...
        if maneuver_mode == 4:#######################目前这个测试只能随机机动方式 若想测试固定机动方式需要在环境的append处修改
...
if __name__ == '__main__':
    ...
    restore_parameters(dqn.eval_net, '4final_20230528_dqn120_1.pkl')
    ...
    df1.to_csv('result/dqn_ZEM.csv', index=False, header=header1)
    df1 = pd.DataFrame(columns=header2)
    df1.to_csv('result/dqn_percentage.csv', index=False, header=header2)
    for maneuver_mode in maneuver_list:
        ...
        df1.to_csv('result/dqn_ZEM.csv', mode='a', index=False, header=False)
        df2 = pd.DataFrame(data=[data2], columns=header2)
        df2.to_csv('result/dqn_percentage.csv', mode='a', index=False, header=False)
    print('finished!')
