...
from hindsight_dqn_train import Normalizer, DQN
import torch
import random

from tensorboardX import SummaryWriter
log_dir1 = 'log\dqn1_test1'
logger1 = SummaryWriter(logdir=log_dir1)

log_dir2 = 'log\dqn1_test2'
logger2 = SummaryWriter(logdir=log_dir2)

log_dir3 = 'log\dqn1_test3'
logger3 = SummaryWriter(logdir=log_dir3)

log_dir4 = 'log\dqn1_test4'
...
def test(maneuver_mode, test_num):
    ...
    env = simulitiveEnv(xt0, yt0, vt0, xm0, ym0, vm0, Initial_heading_angle_receive, distance_maneuver,
                        Value_direction_maneuver, Value_target_acceleration, horizon)
    state = env.reset()
    num_inputs = 6
    ep_reward = 0
    hit_num3 = 0
    ...
    for i in range(test_num):
        ...
        while not done and num_plays < horizon and not interrupt_flag:
            ...
            action_index = dqn.choose_action_greedy(state)
            action = env.choose_action_RL(action_index)
            aRL.append(action)
            state, reward, done, flag = env._step(action)
            # print(state)
...
if __name__ == '__main__':
    ...
    restore_parameters(dqn.eval_net, 'final_20230528_hindsight_dqn120.pkl')
    ...
    df1.to_csv('result/hindsight_dqn_ZEM.csv', index=False, header=header1)
    df1 = pd.DataFrame(columns=header2)
    df1.to_csv('result/hindsight_dqn_percentage.csv', index=False, header=header2)
    for maneuver_mode in maneuver_list:
        ...
        df1.to_csv('result/hindsight_dqn_ZEM.csv', mode='a', index=False, header=False)
        df2 = pd.DataFrame(data=[data2], columns=header2)
        df2.to_csv('result/hindsight_dqn_percentage.csv', mode='a', index=False, header=False)
    print('finished!')
